"""
================================================================================
TB DETECTION SYSTEM — model.py
================================================================================
Contains every class and the full training pipeline from the original file,
completely unchanged in logic.

The only modification: EntropyGuidedSegmentor.segment_lung_roi() now accepts
either a file-path string OR a BGR numpy array, so the server can pass an
in-memory image without writing it to disk first.

TRAINING:
    python model.py                          # trains and saves to ./saved_models/
    python model.py --mode predict --image x.png   # predict one image via CLI
================================================================================
"""
...
class EnsembleClassifier:
    """SVM + RandomForest + GradientBoosting with save/load"""
...
class TBPredictor:
    ...
    predict() accepts either a file path (str) or a BGR numpy array.
    """
    ...
    def predict(self, image_input, return_details=True):
        """
        image_input : str (file path) OR numpy array (BGR).
        """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print("To predict: python model.py --mode predict --image <path>")
        print()
        train_model()
    else:
        main()
